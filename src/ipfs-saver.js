...
IpfsSaver.prototype.save = async function(text, method, callback, options) {
	...
	try {
		...
		try {
			...
			const message = "Failed to transform current document";
			console.log(message);
			console.log(error);
			callback(error, message);
			return false;
		...
		if (unpin != added[0].hash && ($tw.utils.getIpfsProtocol() == "ipns" || ipfsProtocol == "ipns")) {
			...
			if (hash == ipnsKey || ipfsProtocol == "ipfs") {
				...
				if (published == null) {
					console.log(message);
					console.log(error);
				} else if ($tw.utils.getIpfsVerbose()) console.log(message);
			}
		}
		...
		const message = "Unable to save current document to Ipfs";
		console.log(message);	
		console.log(error);
		callback(error, message);
		return false;
...
IpfsSaver.prototype.handleUploadCanonicalUri = async function(self, event) {
	...
	try {
		...
		const message = "Failed to transform attachment";
		console.log(message);
		console.log(error);
		self.errorDialog(message);
		return false;
