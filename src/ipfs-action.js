...
(function() {
  ...
  IpfsAction.prototype.handleIpfsPin = async function(event) {
    try {
      ...
      if (event.param !== undefined && event.param !== null) {
        ...
        // Process _export_uri
        var uri = tiddler.getFieldString("_export_uri");
        if (uri !== undefined && uri !== null && uri.trim() !== "") {
          const parsed = await $tw.ipfs.normalizeIpfsUrl(uri);
          await this.ipfsPin(parsed);
        }
  ...
  IpfsAction.prototype.ipfsPin = async function(uri) {
    // Check
  ...
  IpfsAction.prototype.ipfsUnpin = async function(uri) {
    // Check
    if (uri == undefined || uri == null) {
      throw new Error("Undefined URI...");
    }
