...
SaverHandler.prototype.initSavers = function(moduleType) {
  ...
  $tw.modules.forEachModuleOfType(moduleType,function(title,module) {
    if(module.canSave(self)) {
      self.savers.push({ module: module.create(self.wiki), title: title });
    }
  });
  // Sort savers
  this.sortSavers();
  // Init Preferred Saver if needed
  var tiddler = $tw.wiki.getTiddler("$:/core/saver/preferred-saver");
  if (tiddler == undefined || tiddler == null) {
    // Current saver
    const saver = this.savers[this.savers.length - 1];
    var fields = {
      title: "$:/core/saver/preferred-saver",
      text: saver.title
    };
...
SaverHandler.prototype.updateSaver = function(name, priority) {
  if (priority !== undefined && name !== undefined) {
    ...
    for (var i = 0; i < this.savers.length; i++) {
      const current = this.savers[i].module;
