  ...
  if (ipnsName !== null && ipnsKey !== null) {
    ...
    if (keys !== null && keys !== undefined && Array.isArray(keys)) {
      this.getLogger().info(
        ...
        + " and IPNS key:"
        + "\n "
        + this.ipfsUri.normalizeGatewayUrl(ipfsKeyword + ipnsKey)
      );
    ...
    this.getLogger().info(
      ...
      + "\n "
      + this.ipfsUri.normalizeGatewayUrl(ipnsKeyword + ipnsKey)
    );
  } else {
    var found = false;
    if (keys !== null && keys !== undefined && Array.isArray(keys)) {
...
IpfsWrapper.prototype.generateIpnsKey = async function(ipfs, name) {
  ...
  try {
    ...
    this.getLogger().info(
      ...
      + "\n "
      + this.ipfsUri.normalizeGatewayUrl(ipnsKeyword + key)
    );
    return key;
  } catch (error) {
    this.getLogger().error(error);
...
IpfsWrapper.prototype.addToIpfs = async function(ipfs, content) {
  ...
  try {
    ...
    this.getLogger().info(
      ...
      + " bytes:"
      + "\n "
      + this.ipfsUri.normalizeGatewayUrl(ipfsKeyword + hash)
    );
    return {
      added: hash,
      size: size
...
IpfsWrapper.prototype.resolveIpnsKey = async function(ipfs, key) {
  ...
  try {
    ...
    this.getLogger().info(
      ...
      + "\n "
      + this.ipfsUri.normalizeGatewayUrl(resolved)
    );
    return cid;
  } catch (error) {
    this.getLogger().error(error);
