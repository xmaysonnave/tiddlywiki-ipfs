...
describe('CID', () => {
  ...
  it('Invalid IPFS cid', async () => {
    ...
    expect(ipnsIdentifier == null && protocol === null && cid == null).to.be
      .true
    var { ipnsIdentifier, protocol, cid } = ipfsBundle.decodeCid(
  ...
  it('Decode IPNS name', async () => {
    ...
    expect(cid == null && protocol === 'ipns' && ipnsIdentifier === 'tiddly').to
      .be.true
  })
  it('rsa2048Base58V0 CID', async () => {
    const ipfsBundle = new IpfsBundle()
    ipfsBundle.init()
    expect(ipfsBundle.isCid(rsa2048Base58V0)).to.be.true
  })
  it('ed25519Base58V0 CID', async () => {
    const ipfsBundle = new IpfsBundle()
    ipfsBundle.init()
    expect(ipfsBundle.isCid(ed25519Base58V0)).to.be.true
  })
  it('ed25519DagPbBase32V1 CID', async () => {
    const ipfsBundle = new IpfsBundle()
    ipfsBundle.init()
    expect(ipfsBundle.isCid(ed25519DagPbBase32V1)).to.be.true
  })
  it('ed25519Libp2pKeyBase32V1 CID', async () => {
    const ipfsBundle = new IpfsBundle()
    ipfsBundle.init()
    expect(ipfsBundle.isCid(ed25519Libp2pKeyBase32V1)).to.be.true
  })
  it('ed25519Base36V1 CID', async () => {
    const ipfsBundle = new IpfsBundle()
    ipfsBundle.init()
    expect(ipfsBundle.isCid(ed25519Base36V1)).to.be.true
  })
  ...
  it('convert sha256Base32V1 to sha256Base58V0 CID', async () => {
    ...
    expect(converted === sha256Base58V0).to.be.true
  })
  it('IPFS Protocol', () => {
    const ipfsBundle = new IpfsBundle()
    ipfsBundle.init()
